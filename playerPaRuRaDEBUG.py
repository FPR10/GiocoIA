...
def playerStrategy(game, state, timeout=3):
    ...
    depth     = 1
    reached_depth = 0  # <-- traccia l'ultima profondità completata
    ...
    while True:
        ...
        try:
            ...
            if move is not None:
                best_move = move
            reached_depth = depth          # <-- aggiorna solo se completata
            depth += 1
        except _Timeout:
            break

    print(f"[Zola AI] {state.to_move} → profondità raggiunta: {reached_depth}  |  mossa scelta: {best_move}")
    return best_move